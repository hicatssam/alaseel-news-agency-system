...
@section('breadcrumb') <a href="{{ route('admin.users.index') }}">{{ __('admin.nav_users') }}</a> <span class="sep">›</span> {{ __('admin.btn_add') }} @endsection
@section('content')
...
<div class="card">
  <div class="card-body">
    ...
    <div class="form-row">
      <div class="form-group">
        <label class="form-label">{{ __('admin.label_password') }} <span style="color:#e74c3c">*</span></label>
        <input type="password" name="password" class="form-control" required minlength="8">
      </div>
      <div class="form-group">
        <label class="form-label">{{ __('admin.label_password_confirm') }} <span style="color:#e74c3c">*</span></label>
    ...
    <div class="form-group">
      ...
      <div style="display:flex;flex-wrap:wrap;gap:8px">
        ...
        <label class="form-check" style="background:#f8f9fa;padding:6px 10px;border-radius:6px;border:1px solid #dee2e6">
          <input type="checkbox" name="roles[]" value="{{ $role->id }}" style="accent-color:#c9a84c">
          {{ $role->name }}
        </label>
        @endforeach
      </div>
    ...
    <div class="form-group">
      <label class="form-check"><input type="checkbox" name="status" value="1" checked> {{ __('admin.label_active_user') }}</label>
    </div>
