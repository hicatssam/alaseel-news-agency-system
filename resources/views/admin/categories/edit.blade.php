...
@section('title', __('admin.categories_edit'))
@section('breadcrumb') <a href="{{ route('admin.categories.index') }}">{{ __('admin.nav_categories') }}</a> <span class="sep">›</span> {{ __('admin.btn_edit') }} @endsection
@section('content')
<div class="card" style="max-width:680px">
  <div class="card-header">
    <div class="card-title"><i class="fa-solid fa-folder-pen" style="color:var(--gold)"></i> {{ __('admin.categories_edit') }}: {{ $category->name }}</div>
  ...
  <div class="card-body">
    <form method="POST" action="{{ route('admin.categories.update', $category) }}">
      ...
      <div class="form-row">
        ...
        <div class="form-group">
          <label class="form-label">{{ __('admin.label_slug_field') }}</label>
          <input name="slug" class="form-control" value="{{ old('slug',$category->slug) }}" dir="ltr">
        </div>
      </div>
      <div class="form-row">
        <div class="form-group">
        ...
        <div class="form-group">
          ...
          <input name="color" class="form-control" value="{{ old('color',$category->color) }}" placeholder="#C89A2B" dir="ltr">
        </div>
      </div>
      <div class="form-group">
      ...
      <div class="form-row">
        <div class="form-group">
          ...
          <input name="image" class="form-control" value="{{ old('image',$category->image) }}" placeholder="https://..." dir="ltr">
        </div>
        <div class="form-group">
      ...
      <div style="background:#f8f9fa;border-radius:8px;padding:16px;margin-bottom:16px">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px">
          <label class="form-check">
            <input type="checkbox" name="status" value="1" {{ old('status',intval($category->status))? 'checked':'' }}>
            <span>{{ __('admin.label_active_category') }}</span>
          </label>
          <label class="form-check">
            <input type="checkbox" name="show_in_header" value="1" {{ old('show_in_header',intval($category->show_in_header))? 'checked':'' }}>
            <span>{{ __('admin.label_show_in_header') }}</span>
          </label>
          <label class="form-check">
            <input type="checkbox" name="show_in_footer" value="1" {{ old('show_in_footer',intval($category->show_in_footer))? 'checked':'' }}>
            <span>{{ __('admin.label_show_in_footer') }}</span>
          </label>
          <label class="form-check">
            <input type="checkbox" name="show_on_homepage" value="1" {{ old('show_on_homepage',intval($category->show_on_homepage))? 'checked':'' }}>
      ...
      <div style="display:flex;gap:10px">
        <button type="submit" class="btn btn-primary"><i class="fa-solid fa-floppy-disk"></i> {{ __('admin.btn_save_changes') }}</button>
